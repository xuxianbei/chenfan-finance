...
public class BeanUtilCopy extends BeanUtils {
    ...
    public static <S, T> T copyPropertiesIgnoreType(S source, Class<T> target) {
        return JSONObject.parseObject(JSONObject.toJSONString(source), target);
    }

    public static <S, T> List<T> copyListPropertiesIgnoreType(List<S> sources, Class<T> target) {
        List<T> list = new ArrayList<>(sources.size());
       for (S s:sources){
            list.add(copyPropertiesIgnoreType(s,target));
       }
       return list;
    }
